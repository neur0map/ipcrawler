...
impl DigPlugin {
    ...
    async fn run_dig(
        ...
        self.send_log(
            ui_sender,
            "INFO",
            &format!("Running dig {} query for {}", record_type, target),
        );
            ...
            Ok(command_result) => {
                if command_result.exit_code == 0 {
                    self.send_log(
                        ui_sender,
                        "INFO",
                        &format!("dig {} query completed for {}", record_type, target),
                    );
                    Ok(command_result.stdout)
                } else {
                    let error_msg = format!(
                        "dig {} query failed for {} (exit code {}): {}",
    ...
    fn write_plugin_results(
        ...
        let result_file = scans_dir.join("dig_results.txt");
...
impl PortScan for DigPlugin {
    ...
    async fn run(&self, state: &mut RunState, config: &GlobalConfig) -> Result<Vec<Service>> {
        ...
        self.send_log(
            ui_sender,
            "INFO",
            &format!("Starting dig DNS queries for target: {}", target),
        );
        ...
        if !all_results.is_empty() {
            if let Err(e) = self.write_plugin_results(&all_results, &dirs.scans, target) {
                self.send_log(
                    ui_sender,
                    "WARN",
                    &format!("Failed to write dig results to file: {}", e),
                );
            } else {
        ...
        self.send_log(
            ...
            "INFO",
            &format!(
                "dig completed successfully. Found {} DNS record types",
                services.len()
            ),
        );
        Ok(services)
