...
impl PortScannerPlugin {
    ...
    async fn execute_rustscan(&self, target: &str, config: &GlobalConfig, ui_sender: &mpsc::UnboundedSender<UiEvent>) -> Result<Vec<u16>> {
        // Check if rustscan is available
        if which::which("rustscan").is_err() {
            self.send_log(ui_sender, "ERROR", "RustScan not available - port scanner requires RustScan for port discovery");
            return Err(anyhow::anyhow!("Port scanner requires RustScan for port discovery. Install with: cargo install rustscan"));
        }
        
        self.send_log(ui_sender, "INFO", &format!("Starting RustScan fast port discovery for {}", target));
        
        let port_scanner_config = config.tools.as_ref()
            .and_then(|tools| tools.port_scanner.as_ref());
        
        // Get configuration or use defaults
        let (enabled, base_args, timeout_ms, total_timeout_ms, batch_size, ulimit, tries) = if let Some(port_config) = port_scanner_config {
            let rs_config = &port_config.rustscan;
            (port_config.enabled, rs_config.base_args.clone(), rs_config.timeout_ms, rs_config.total_timeout_ms, rs_config.batch_size, rs_config.ulimit, rs_config.tries)
        } else {
            self.send_log(ui_sender, "WARN", "Port scanner configuration not found in global.toml, using defaults");
            // Default values
            (true, vec!["--greppable".to_string(), "--no-config".to_string()], 3000, 300000, 4500, 5000, 1)
        };
        ...
        args.extend(vec![
            "-a".to_string(), target.to_string(),  // Target address
        ]);
        ...
        args.extend(vec![
            ...
            "--tries".to_string(), tries.to_string(),
        ...
        match execute(command, &args_str, Path::new("."), Some(total_timeout_ms)).await {
            Ok(command_result) => {
                ...
                if command_result.exit_code == 0 {
                    ...
                    if ports.is_empty() {
                        self.send_log(ui_sender, "WARN", &format!("RustScan found no open ports for {}", target));
                    } else {
                        self.send_log(ui_sender, "INFO", &format!("RustScan discovered {} open ports for {}", ports.len(), target));
                    }
                    Ok(ports)
                } else {
                    let error_msg = format!("RustScan failed for {} (exit code {}): {}", target, command_result.exit_code, command_result.stderr);
    ...
    async fn execute_nmap(&self, target: &str, ports: &[u16], config: &GlobalConfig, ui_sender: &mpsc::UnboundedSender<UiEvent>) -> Result<Vec<Service>> {
        if ports.is_empty() {
            return Ok(Vec::new());
        }

        self.send_log(ui_sender, "INFO", &format!("Starting Nmap service detection for {} ports on {}", ports.len(), target));
        tracing::info!("Starting Nmap service detection for {} ports on {}", ports.len(), target);
        ...
        let has_sudo = if nmap_config.auto_detect_sudo {
            ...
            if sudo_detected {
                self.send_log(ui_sender, "INFO", "Sudo privileges detected - using advanced Nmap scans");
            } else {
                self.send_log(ui_sender, "INFO", "No sudo privileges - using connect scans and service detection");
            }
            sudo_detected
        } else {
            false
        ...
        args.extend(vec![
            "-p".to_string(), port_list,  // Only scan discovered ports
        ...
        if has_sudo {
            ...
            if nmap_config.fallback_to_connect_scan {
                ...
                return Err(anyhow::anyhow!("Nmap requires sudo privileges and fallback is disabled"));
            }
        }
        ...
        args.extend(vec![
            ...
            "--max-retries".to_string(), nmap_config.max_retries.to_string(),
            "--host-timeout".to_string(), format!("{}s", nmap_config.host_timeout_sec),
        ]);

        // Add service detection intensity if configured
        if nmap_config.version_intensity > 0 {
            args.extend(vec!["--version-intensity".to_string(), nmap_config.version_intensity.to_string()]);
        }
        ...
        self.send_log(ui_sender, "INFO", &format!("Executing Nmap: {} {}", nmap_config.command, args.join(" ")));
        tracing::info!("Executing Nmap: {} {}", nmap_config.command, args.join(" "));
        
        match execute(&nmap_config.command, &args_str, Path::new("."), Some(nmap_config.total_timeout_ms)).await {
            Ok(command_result) => {
                if command_result.exit_code == 0 {
                    let services = self.parse_nmap_output(&command_result.stdout, target);
                    self.send_log(ui_sender, "INFO", &format!("Nmap completed service detection for {} - found {} services", target, services.len()));
                    Ok(services)
                } else {
                    let error_msg = format!("Nmap failed for {} (exit code {}): {}", target, command_result.exit_code, command_result.stderr);
                    ...
                    if !has_sudo && command_result.stderr.contains("requires root privileges") {
                        self.send_log(ui_sender, "INFO", "Nmap needs sudo for some features - continuing with available data");
                    }
    ...
    fn parse_nmap_output(&self, output: &str, target: &str) -> Vec<Service> {
        ...
        for line in output.lines() {
            ...
            if line.is_empty() || line.starts_with("Starting") || line.starts_with("Nmap") || line.starts_with("Not shown") {
                continue;
            }
    ...
    fn parse_nmap_service_line(&self, line: &str, target: &str) -> Option<Service> {
        ...
        let secure = service_name.contains("ssl") || 
                    ...
                    port == 443 || port == 22 || port == 993 || port == 995;
    ...
    fn write_results(&self, rustscan_ports: &[u16], nmap_services: &[Service], scans_dir: &Path, target: &str) -> Result<()> {
        ...
        if nmap_services.is_empty() {
            ...
            for service in nmap_services {
                ...
                content.push_str(&format!("{}/{} open {} on {}{}\n", 
        ...
        // Summary Statistics
        let tcp_services = nmap_services.iter().filter(|s| matches!(s.proto, Proto::Tcp)).count();
        let udp_services = nmap_services.iter().filter(|s| matches!(s.proto, Proto::Udp)).count();
        let secure_services = nmap_services.iter().filter(|s| s.secure).count();
        
        content.push_str("=== Summary Statistics ===\n");
        content.push_str(&format!("Total ports discovered: {}\n", rustscan_ports.len()));
        content.push_str(&format!("Services detected: {}\n", nmap_services.len()));
...
impl PortScan for PortScannerPlugin {
    ...
    async fn run(&self, state: &mut RunState, config: &GlobalConfig) -> Result<Vec<Service>> {
        ...
        self.send_log(ui_sender, "INFO", &format!("Starting two-phase port scanner for target: {}", target));
        ...
        let nmap_services = if !rustscan_ports.is_empty() {
            ...
            match self.execute_nmap(target, &rustscan_ports, config, ui_sender).await {
                Ok(services) => {
                ...
                Err(_) => {
                    ...
                    self.send_log(ui_sender, "WARN", "Nmap service detection failed, but port discovery succeeded");
                    
                    // Create basic services from port discovery for return value
                    rustscan_ports.iter().map(|&port| Service {
                        proto: Proto::Tcp,
                        port,
        ...
        if let Err(e) = self.write_results(&rustscan_ports, &nmap_services, &dirs.scans, target) {
            self.send_log(ui_sender, "WARN", &format!("Failed to write port scanner results: {}", e));
        } else {
            self.send_log(ui_sender, "INFO", "Port scanner results written to scans directory");
        }
        ...
        let final_services = if nmap_services.is_empty() && !rustscan_ports.is_empty() {
            self.send_log(ui_sender, "INFO", "Creating basic service entries from discovered ports");
            rustscan_ports.iter().map(|&port| Service {
                proto: Proto::Tcp,
                port,
                name: "unknown".to_string(),
        ...
        self.send_log(ui_sender, "INFO", &format!("Port scanner completed successfully. Found {} ports with {} services", rustscan_ports.len(), final_services.len()));
