...
#[allow(dead_code)]
pub enum TaskUpdate {
    Started { task_id: TaskId, tool_name: String, target: String, port: Option<u16> },
    Completed { task_id: TaskId, result: String },
    Failed { task_id: TaskId, error: String },
    Progress { queued: usize, running: usize, completed: usize },
}
...
impl TaskRunner {
    ...
    async fn execute_task(
        ...
        if parts.is_empty() {
            ...
            let failed_status = TaskStatus::Failed { error: error.clone() };
            state.lock().await.insert(task.id.clone(), failed_status.clone());
        ...
        let (status, stdout, stderr) = match command_result {
            Ok(Ok(output)) => {
                ...
                let status = TaskStatus::Completed { duration, exit_code };
            ...
            Ok(Err(e)) => {
                ...
                (TaskStatus::Failed { error: error.clone() }, String::new(), error)
            }
            Err(_) => {
