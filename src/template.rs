...
impl TemplateEngine {
    pub fn new(templates_dir: PathBuf) -> Result<Self, TemplateError> {
        ...
        let engine = Self { env, templates_dir };
    ...
    fn get_embedded_template(&self, format: &OutputFormat, template_name: &str) -> String {
        match (format, template_name) {
            (OutputFormat::Json, "summary") => {
                include_str!("../templates/json/summary.json.j2").to_string()
            }
            (OutputFormat::Text, "summary") => {
                include_str!("../templates/txt/summary.txt.j2").to_string()
            }
            (OutputFormat::Markdown, "summary") => {
                include_str!("../templates/markdown/summary.md.j2").to_string()
            }
            (OutputFormat::Html, "summary") => {
                include_str!("../templates/html/summary.html.j2").to_string()
            }
    ...
    fn get_default_template(&self, format: &OutputFormat) -> String {
        match format {
            ...
            OutputFormat::Markdown => {
                include_str!("../templates/markdown/default.md.j2").to_string()
            }
    ...
    fn create_template_context(&self, summary: &ScanSummary) -> JsonValue {
        ...
        if let Some(context_obj) = context.as_object_mut() {
            ...
            context_obj.insert(
                "discoveries_by_type".to_string(),
                json!(discoveries_by_type),
            );

            // Add convenience fields
            context_obj.insert(
                "ports_count".to_string(),
                json!(
                    summary
                        .discoveries
                        .iter()
                        .filter(|d| matches!(
                            d.discovery_type,
                            crate::output::DiscoveryType::Port { .. }
                        ))
                        .count()
                ),
            );
            ...
            context_obj.insert(
                "template_generated_at".to_string(),
    ...
    fn group_discoveries_by_type<'a>(
        &self,
        summary: &'a ScanSummary,
    ) -> HashMap<String, Vec<&'a crate::output::Discovery>> {
