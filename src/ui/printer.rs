...
pub fn print_summary(state: &RunState) {
    ...
    println!("{}", "═══════════════════════════════════════".with(Color::Green).bold());
    
    println!("\n{}: {}", "Target".with(Color::Cyan).bold(), state.target);
    println!("{}: {}", "Run ID".with(Color::Cyan).bold(), state.run_id);
    
    println!("\n{}", "Results:".with(Color::Yellow).bold());
    println!("  Open Ports: {}", state.ports_open.len().to_string().with(Color::Green).bold());
    println!("  Services: {}", state.services.len().to_string().with(Color::Green).bold());
    ...
    if !state.errors.is_empty() {
        println!("  Errors: {}", state.errors.len().to_string().with(Color::Red).bold());
    }
