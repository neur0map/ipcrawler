...
impl ReconPaths {
    ...
    pub fn resolve_config(&self, name_or_path: &str) -> Result<PathBuf, String> {
        ...
        if name_or_path.contains('/') || name_or_path.contains('\\') {
            ...
            if path.exists() {
                ...
                return Err(format!(
                    "Config file not found: {} (tried with and without .yaml extension)",
                    name_or_path
                ));
            }
        }
        ...
        Err(format!(
            "Config profile '{}' not found in any location",
            name_or_path
        ))
    }
    ...
    pub fn list_available_configs(&self) -> Vec<(String, PathBuf)> {
        ...
        self.scan_directory(
            &self.user_config.join("profiles"),
            "User Profiles",
            &mut configs,
        );
        ...
        self.scan_directory(
            &self.system_templates.join("templates"),
            "System Templates",
            &mut configs,
        );
