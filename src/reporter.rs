...
- **{{ tool_result.template.name }}** - {{ tool_result.target }}
  {% if tool_result.success %}✅{% else %}❌{% endif %}
{% endfor %}
...
"#
    ...
    pub async fn generate_reports(&self, results: Vec<tokio::task::JoinHandle<(ToolTemplate, String, Result<()>)>>) -> Result<()> {
        ...
        println!("{} Summary report: reports/ipcrawler_summary_{}.md", "📄".blue(), timestamp);
        println!("{} Detailed report: reports/ipcrawler_detailed_{}.md", "📄".blue(), timestamp);
