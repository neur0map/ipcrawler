...
impl TemplateManager {
    ...
    pub fn get_default_scan_templates(&self) -> Vec<&ToolTemplate> {
        // Return templates for default reconnaissance scan
        let mut default_templates = vec![];
        
        // Port scanning
        if let Some(nmap) = self.get_template("nmap_default") {
            default_templates.push(nmap);
        } else if let Some(rustscan) = self.get_template("rustscan_default") {
            default_templates.push(rustscan);
        }
        
        // DNS discovery
        if let Some(dig) = self.get_template("dig_basic") {
            default_templates.push(dig);
        }
        
        // Ping test
        if let Some(ping) = self.get_template("ping_test") {
            default_templates.push(ping);
        }
        
        default_templates
    }

    pub fn get_comprehensive_scan_templates(&self) -> Vec<&ToolTemplate> {
        // Return templates for comprehensive scan
        let mut comprehensive_templates = self.get_default_scan_templates();
        
        // Add additional tools
        if let Some(whois) = self.get_template("whois_lookup") {
            comprehensive_templates.push(whois);
        }
        
        if let Some(sslscan) = self.get_template("ssl_scan") {
            comprehensive_templates.push(sslscan);
        }
        
        comprehensive_templates
    }

    pub fn get_stealth_scan_templates(&self) -> Vec<&ToolTemplate> {
        // Return templates for stealthy scanning
        let mut stealth_templates = vec![];
        
        // Use passive/quiet tools
        if let Some(dig) = self.get_template("dig_passive") {
            stealth_templates.push(dig);
        }
        
        if let Some(whois) = self.get_template("whois_lookup") {
            stealth_templates.push(whois);
        }
        
        // Only use stealthy port scanning
        if let Some(nmap_stealth) = self.get_template("nmap_stealth") {
            stealth_templates.push(nmap_stealth);
        }
