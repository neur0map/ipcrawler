...
use crate::error_handler::{EmergencyStopError, ErrorContext, ErrorHandler};
...
impl Executor {
    ...
    pub async fn execute_batch(
        &mut self,
        _discovery_bar: &ProgressBar,
    ) -> Result<Vec<ToolResult>, Box<dyn std::error::Error + Send + Sync>> {
        self.log_info("Starting tool execution").await?;

        let enabled_tools: Vec<Tool> = self
            .config
            .tools
            .clone()
            .into_iter()
        ...
        if enabled_tools.len() > 1 {
            ...
            self.progress_manager.print_verbose_info(
        ...
        while !active_tasks.is_empty() {
            ...
            let (result, _task_index, remaining_tasks) =
            ...
            match result {
                Ok(Ok(tool_result)) => {
                    ...
                    let queue_info = if !pending_tools.is_empty() {
                        let queue_names: Vec<String> =
                            pending_tools.iter().map(|t| t.name.clone()).collect();
                        format!(" | Queue: {}", queue_names.join(", "))
                    } else {
                        String::new()
                    };
                    ...
                    if tool_result.exit_code == 0 {
                        self.progress_manager.print_verbose_success(
                            &format!("{} {}{}", tool_result.tool_name, duration, queue_info),
                            self.context.verbose,
                        );
                    } else {
                        ...
                        self.progress_manager.print_error(&format!(
                            "{} {}{}",
                            tool_result.tool_name, duration, queue_info
                        ));
                        ...
                        if should_stop {
                            ...
                            self.progress_manager.print_error(
                                "Emergency stop triggered - cancelling all remaining tools",
                            );
                            ...
                            return Err(Box::new(EmergencyStopError {
                                message: "Critical tool failure triggered emergency stop"
                                    .to_string(),
                    ...
                    if let Err(_) = crate::validator::MemoryValidator::validate_file_size(
                        &tool_result.stdout_file,
                        50 * 1024 * 1024,
                    ) {
                        // 50MB limit
                        self.log_error(&format!(
                            "Tool {} output file too large, skipping parsing",
                        ...
                        {
                            self.log_error(&format!(
                                "Tool {} output contains invalid characters, skipping parsing",
                                tool_result.tool_name
                            ))
                            .await?;
                        } else {
                            ...
                            let parse_result =
                                parser.parse_output(&content, &tool_result.tool_name);

                            // Memory safety validation for large discovery collections
                            if let Err(_) =
                                crate::validator::MemoryValidator::validate_collection_size(
                                    &parse_result.discoveries,
                                    10000,
                                )
                            {
                                self.log_error(&format!(
                                    "Tool {} produced excessive discoveries, truncating results",
                                    tool_result.tool_name
                                ))
                                .await?;
                            }
                            ...
                            for discovery in &parse_result.discoveries {
                                match &discovery.discovery_type {
                                    ...
                                    crate::output::DiscoveryType::Vulnerability { .. } => {
                                        vuln_count += 1
                                    }
                                    _ => {}
                                }
                            ...
                            if port_count > 0 {
                                ...
                                let ports: Vec<u16> = parse_result
                                    .discoveries
                                    .iter()
                                    .filter_map(|d| match &d.discovery_type {
                                        crate::output::DiscoveryType::Port { number, .. } => {
                                            Some(*number)
                                        }
                                        _ => None,
                                    })
                            ...
                            if service_count > 0 {
                                ...
                                let services: Vec<String> = parse_result
                                    .discoveries
                                    .iter()
                                    .filter_map(|d| match &d.discovery_type {
                                        crate::output::DiscoveryType::Service {
                ...
                Ok(Err(e)) => {
                    self.log_error(&format!("Tool execution failed: {}", e))
                        .await?;
        ...
        self.log_info(&format!(
            "Execution complete. {} tools executed",
    ...
    async fn spawn_tool_task(
        ...
        tokio::spawn(async move {
            ...
            Self::execute_tool_with_retry(tool, context, progress_manager, log_file, retry_count)
                .await
        })
    }
    ...
    async fn execute_tool_with_retry(
        ...
        loop {
            ...
            pb.set_message(format!(
                "{}: scanning{}{}",
                gradient_tool(&tool.name),
                port_info,
                attempt_msg
            ));

            match Self::execute_single_tool(
                tool.clone(),
                context.clone(),
                &pb,
                log_file.clone(),
    ...
    async fn execute_single_tool(
        ...
        let stdout_file = context
            .output_dir
            .join("raw")
            .join(format!("{}.out", tool.name));
        let stderr_file = context
            .output_dir
            .join("errors")
            .join(format!("{}.err", tool.name));
        ...
        let exit_status = match timeout(timeout_duration, child.wait()).await {
            ...
            Err(_) => {
                Self::write_log(
                    &log_file,
                    &format!("Tool {} timed out after {}s", tool.name, tool.timeout),
                )
                .await?;
                child.kill().await?;
        ...
        Self::write_log(
            &log_file,
            &format!(
                "Tool {} completed in {:?} with exit code {}",
                tool.name, duration, exit_code
            ),
        )
        .await?;
    ...
    pub fn print_summary(&self, results: &[ToolResult]) {
        ...
        if failed > 0 {
            ...
            for result in results.iter().filter(|r| r.exit_code != 0) {
                self.progress_manager
                    .print_error(&format!("  {}", result.tool_name));
            }
        }
        ...
        if self.error_handler.has_errors() {
            if let Ok(error_summary) = self.error_handler.create_error_summary() {
                self.progress_manager
                    .print_warning("Error details logged - see full summary below:");
