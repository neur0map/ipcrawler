use crate::config::{Chain, Config, Tool};
use crate::executor::{ExecutionContext, Executor, ToolResult};
...
impl Pipeline {
    ...
    pub async fn execute_with_chaining(
        ...
        let mut remaining_tools: Vec<Tool> = self
            .config
            .tools
            .clone()
            .into_iter()
        ...
        while !remaining_tools.is_empty() {
            ...
            if ready_tools.is_empty() {
                self.progress_manager
                    .print_warning("No tools ready - dependency issue");
            ...
            if !waiting_names.is_empty() {
                ...
                self.progress_manager.print_verbose_info(
                    &format!("Running: {}", ready_names.join(", ")),
                    self.context.verbose,
                );
            }
        ...
        if let Some(dependencies) = deps.get(&tool.name) {
            for dep in dependencies {
                ...
                if let Some(chain) = self
                    ...
                    .chains
                    .iter()
                    .find(|c| c.from == *dep && c.to == tool.name)
                {
    ...
    fn evaluate_chain_condition(&self, chain: &Chain, result: &ToolResult) -> bool {
        match chain.condition.as_str() {
            ...
            _ => {
                self.progress_manager
                    .print_warning(&format!("Unknown chain condition: {}", chain.condition));
                false
            }
        }
    }
            ...
            if chain.from == result.tool_name {
                if let Some(target_tool) = waiting_tools.iter_mut().find(|t| t.name == chain.to) {
                    if self.evaluate_chain_condition(chain, result) {
                        self.progress_manager.print_verbose_info(
                            &format!("Chain: {} → {}", chain.from, chain.to),
                            self.context.verbose,
                        );
    ...
    fn update_tool_with_chain_data(
        &self,
        tool: &mut Tool,
        ...
        if tool.command.contains("{discovered_ports}") {
            ...
            if !ports.is_empty() {
                ...
                self.progress_manager.print_info(&format!(
                    "Passing {} discovered ports from {} → {}: {}",
                    ports.len(),
                    gradient_tool(&source_result.tool_name),
                ...
                if self.context.debug {
                    self.progress_manager
                        .print_verbose_info(&format!("Updated command: {}", tool.command), true);
                }
            } else {
                // If no ports found but tool expects them, warn and disable
                self.progress_manager.print_warning(&format!(
                    "No ports found from {} → {} (disabling tool)",
                    source_result.tool_name, tool.name
                ));
                tool.enabled = false;
            }
        }
