...
async fn execute_single_config(
    ...
    if has_chains {
        let mut pipeline = Pipeline::new(config, context, progress_manager.clone(), emergency_stop, notifications);
        match pipeline.execute_with_chaining().await {
            Ok(res) => Ok(res),
            Err(e) => Err(format!("Pipeline failed for {}: {}", config_name, e).into()),
        }
    } else {
        let mut executor = match Executor::new(config, context, progress_manager, emergency_stop, notifications) {
            Ok(exec) => exec,
            Err(e) => return Err(format!("Failed to create executor for {}: {}", config_name, e).into()),
        };
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let output_base = if let Some(custom_path) = &args.output {
        ...
        if args.debug {
            progress.print_info(&format!("Using custom output path: {}", custom_path.display()));
        }
        custom_path.clone()
    } else {
        ...
        if args.debug {
            let mode = if paths.development_output_dir() == default_dir { "development" } else { "production" };
            progress.print_info(&format!("Using default output path ({}): {}", mode, default_dir.display()));
        }
        default_dir
    };
    ...
    if args.verbose {
        ...
        if args.verbose {
            ...
            let _total_tools: usize = configs.iter().map(|c| c.tools.iter().filter(|t| t.enabled).count()).sum();
            progress.print_info(&format!("  Total enabled tools: {}", _total_tools));
            let _has_chains = configs.iter().any(|c| !c.chains.is_empty());
            progress.print_info(&format!("  Execution mode: {}", if _has_chains { "chained" } else { "parallel" }));
    ...
    for (i, config) in configs.into_iter().enumerate() {
        ...
        let task = tokio::spawn(async move {
            execute_single_config(config, context_clone, config_name, progress_clone, emergency_stop, notifications).await
        });
        ...
        if let Ok(summary) = report_generator.generate_summary_report(&results) {
            ...
            for (format, name) in formats {
                if let Ok(report_path) = report_generator.save_report(&summary, format) {
                    if args.verbose {
                        progress.print_info(&format!("{} report -> {}", name, report_path.display()));
                    }
                }
            }
        }
        ...
        if let Ok(summary) = report_generator.generate_summary_report(&results) {
            ...
            for discovery in &summary.discoveries {
                match &discovery.discovery_type {
                    ...
                    crate::output::DiscoveryType::Service { port, protocol, name, version } => {
        ...
        if doctor.is_see_available() {
            ...
            if progress.prompt_user_yn("Do you want to view the markdown summary?", false) {
                ...
                let success = if cfg!(target_os = "macos") {
                    ...
                    if std::process::Command::new("which").arg("gnome-terminal").output().map(|o| o.status.success()).unwrap_or(false) {
                        std::process::Command::new("gnome-terminal")
                            ...
                            .spawn()
                            .is_ok()
                    } else if std::process::Command::new("which").arg("xterm").output().map(|o| o.status.success()).unwrap_or(false) {
                ...
                if success {
                    if cfg!(target_os = "macos") || cfg!(target_os = "linux") {
                        progress.print_success("Markdown summary opened in new terminal window (130x60)");
                    } else {
                        ...
                        .status()
                    {
                        ...
                        Err(e) => {
                            ...
                            progress.print_info(&format!("You can manually view: {}", markdown_summary.display()));
                        }
                    }
                }
            }
    ...
    if args.debug {
        ...
        progress.print_info(&format!("  Output Directory: {}", output_dir.display()));
        progress.print_info(&format!("  Debug Mode: {}", if args.debug { "Enabled" } else { "Disabled" }));
        progress.print_info(&format!("  Verbose Mode: {}", if args.verbose { "Enabled" } else { "Disabled" }));
        progress.print_info(&format!("  Tools executed: {}", results.len()));
        progress.print_info(&format!("  Total duration: {:.2}s", results.iter().map(|r| r.duration).sum::<std::time::Duration>().as_secs_f64()));
    }
    
    Ok(())
}

fn create_output_structure(target: &str, base_dir: &PathBuf) -> Result<PathBuf, Box<dyn std::error::Error>> {
    ...
    writeln!(file, "[{}] Scan initialized for target: {}", timestamp(), target)?;
    writeln!(file, "[{}] Output directory: {}", timestamp(), output_path.display())?;
...
fn print_chain_summary(results: &[executor::ToolResult], progress: &ProgressManager, verbose: bool) {
    if verbose {
        ...
        if !failed.is_empty() {
            ...
            for result in failed {
                progress.print_error(&format!("    {} (exit code: {})", result.tool_name, result.exit_code));
        ...
        progress.print_info(&format!("Total execution time: {:.2}s", total_duration.as_secs_f64()));
    }
}
...
fn print_path_info(paths: &ReconPaths, args: &cli::Cli) {
    ...
    println!("{}", "[PATHS] Directory Paths Information".bright_cyan().bold());
    println!();
    ...
    if let Ok(exe_path) = std::env::current_exe() {
        println!("   Executable: {}", exe_path.display().to_string().bright_white());
    ...
    if profiles_dir.exists() {
        if let Ok(entries) = std::fs::read_dir(&profiles_dir) {
            ...
            println!("     Status: {} ({} user profiles)", "[ACTIVE]".green(), count);
        } else {
            println!("     Status: {} (directory exists but unreadable)", "[WARNING]".yellow());
        }
    } else {
        println!("     Status: {} (will be created when needed)", "[NOT_CREATED]".yellow());
    }
    println!();
    ...
    if paths.system_templates.exists() {
        if let Ok(entries) = std::fs::read_dir(&paths.system_templates) {
            ...
            println!("     Status: {} ({} templates available)", "[ACTIVE]".green(), count);
        } else {
            println!("     Status: {} (directory exists but unreadable)", "[WARNING]".yellow());
        }
    } else {
        println!("     Status: {} (using development fallback)", "[DEVELOPMENT]".cyan());
    }
    println!();
    ...
    let output_base = args.output.as_ref()
        .map(|p| p.clone())
    ...
    if output_base.exists() {
        if let Ok(entries) = std::fs::read_dir(&output_base) {
            let count = entries.count();
            println!("     Status: {} ({} scan results)", "[ACTIVE]".green(), count);
        } else {
            println!("     Status: {} (directory exists but unreadable)", "[WARNING]".yellow());
        }
    } else {
        println!("     Status: {} (will be created on first scan)", "[NOT_CREATED]".yellow());
    }
    println!();
    ...
    if !is_custom {
        ...
        println!("   {} The tool automatically selects based on execution context", "[INFO]".bright_blue());
    ...
    println!("   {} --target example.com --config default", "ipcrawler".bright_cyan());
    println!("   ");
    println!("   # Create user config directory:");
    println!("   {} -p {}", "mkdir".bright_cyan(), paths.user_config.join("profiles").display());
    println!("   ");
    println!("   # Copy system template to user directory:");
    println!("   {} {} {}", "cp".bright_cyan(), 
        paths.system_templates.join("default.yaml").display(),
        paths.user_config.join("profiles").join("my-custom.yaml").display());
    println!();
    
    println!("{}", "[INFO] For more details, use: ipcrawler --config nonexistent --validate".bright_black());
}
...
fn print_tools_info(config: &Config) {
    ...
    for (i, tool) in config.tools.iter().enumerate() {
        ...
        let status_color = if tool.enabled { status.green() } else { status.red() };
    ...
    println!("   Max Concurrent: {}", config.globals.max_concurrent.to_string().bright_white());
    println!("   Retry Count: {}", config.globals.retry_count.to_string().bright_white());
    println!("   Log Level: {}", config.globals.log_level.bright_white());
}

fn print_dry_run_info(config: &Config, target: &str, output_dir: &PathBuf, progress: &ProgressManager) {
    progress.print_section("Execution Plan");
    ...
    progress.print_section(&format!("Tools to Execute ({} tools):", enabled_tools.len()));
    ...
    if !config.chains.is_empty() {
        ...
        while !remaining_tools.is_empty() && round <= 10 { // Prevent infinite loop
    ...
    progress.print_subsection("Max Concurrent:", &config.globals.max_concurrent.to_string());
...
async fn handle_list_configs(paths: &ReconPaths) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("{}", "[CONFIGS] Available Configuration Profiles".bright_cyan().bold());
    println!();
    ...
    for (group_index, group_name) in group_order.iter().enumerate() {
        if let Some(configs) = config_groups.get(*group_name) {
            ...
            let group_connector = if group_index == group_order.len() - 1 
                && !group_order.iter().skip(group_index + 1).any(|g| config_groups.contains_key(*g)) {
                "└──"
            } else {
                "├──"
            };
            ...
            for (config_index, (name, path, config_opt)) in configs.iter().enumerate() {
                ...
                if let Some(config) = config_opt {
                    if !config.metadata.description.is_empty() {
                        print!(" {}", format!("- {}", config.metadata.description).bright_black());
                    }
                    if !config.metadata.version.is_empty() {
                        print!(" {}", format!("(v{})", config.metadata.version).bright_purple());
                    }
                }
                println!();
    ...
    println!();
    println!("{} {} Found {} configuration profiles across {} locations", 
        "[SUMMARY]".bright_blue(),
    ...
    // Usage examples
    println!();
    println!("{} {} Usage examples:", "[TIPS]".bright_yellow(), "Tips:".bright_white().bold());
...
async fn handle_update() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("{} Current version: {}", "[VERSION]".bright_blue(), current_version.bright_white());
    ...
    if is_cargo_installed {
        // Update via cargo
        println!("{} Updating via cargo install...", "[UPDATING]".bright_yellow());
        ...
        match output {
            Ok(result) => {
                if result.status.success() {
                    println!("{} ipcrawler updated successfully!", "[SUCCESS]".bright_green());
                    println!();
                    println!("{} Run 'ipcrawler --version' to verify the update", "[INFO]".bright_blue());
                } else {
                    // Try from git if crates.io fails
                    println!("{} Cargo install failed, trying from source...", "[WARNING]".bright_yellow());
                    
                    let git_output = Command::new("cargo")
                        .args(&["install", "--force", "--git", "https://github.com/your-username/ipcrawler-rust.git"])
                        .output();
                        
                    match git_output {
                        Ok(git_result) if git_result.status.success() => {
                            println!("{} ipcrawler updated successfully from source!", "[SUCCESS]".bright_green());
                        }
                        _ => {
                            println!("{} Failed to update from source", "[ERROR]".bright_red());
                            println!("{} Please update manually:", "[INFO]".bright_blue());
                            println!("    cargo install --force --git https://github.com/your-username/ipcrawler-rust.git");
                        }
                    }
                }
            }
            Err(e) => {
                ...
                println!("{} Please ensure cargo is in your PATH", "[INFO]".bright_blue());
            }
        }
    } else if is_system_installed {
        // Update via install script
        println!("{} System installation detected", "[DETECTED]".bright_blue());
        ...
        match output {
            Ok(result) => {
                if result.status.success() {
                    ...
                    println!("{} You can manually update by running:", "[INFO]".bright_blue());
                    println!("    curl -sSL https://install.ipcrawler.io | bash");
                }
            }
            Err(e) => {
                println!("{} Failed to download update script: {}", "[ERROR]".bright_red(), e);
                println!("{} Please run the installer manually:", "[INFO]".bright_blue());
                println!("    curl -sSL https://install.ipcrawler.io | bash");
            }
        }
        ...
        println!("{} Development version detected", "[DETECTED]".bright_blue());
        println!("{} To update, pull latest changes and rebuild:", "[INFO]".bright_blue());
        println!("    git pull");
...
async fn handle_resume_scan(resume_dir: &PathBuf, _paths: &ReconPaths, _args: &cli::Cli) -> Result<(), Box<dyn std::error::Error>> {
    println!("{}", "[RESUME] Scan Resumption".bright_cyan().bold());
    println!();
    
    if !resume_dir.exists() {
        println!("{} Resume directory does not exist: {}", "[ERROR]".bright_red(), resume_dir.display());
        return Ok(());
    }
    ...
    if json_report.exists() {
        println!("{} Found existing scan report: {}", "[FOUND]".bright_green(), json_report.display());
    ...
    if logs_dir.exists() {
        println!("{} Found logs directory: {}", "[LOGS]".bright_blue(), logs_dir.display());
    }
    
    if raw_dir.exists() {
        if let Ok(entries) = fs::read_dir(&raw_dir) {
            let file_count = entries.count();
            println!("{} Found {} raw output files in: {}", "[FILES]".bright_yellow(), file_count, raw_dir.display());
        }
    }
    
    println!();
    println!("{} Scan resumption is not yet fully implemented", "[WIP]".bright_yellow());
    println!("{} This feature will allow continuing interrupted scans", "[INFO]".bright_blue());
    println!("{} For now, you can review the existing results above", "[INFO]".bright_blue());
