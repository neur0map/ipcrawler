...
class QuickTCPPortScan(PortScan):
    ...
    async def run(self, target):
        ...
        # Check if we already have discovered TCP services from previous scans
        if hasattr(target, 'services') and target.services:
            tcp_services = [s for s in target.services if s.startswith('tcp/')]
            if tcp_services:
                target.info(f"🔄 Skipping top TCP ports scan - {len(tcp_services)} TCP services already discovered", verbosity=1)
                return []
        ...
        # Default to parallel scan for better performance and reliability
        ...
        if self.get_option("single-scan"):
            ...
            target.info("🚀 Using parallel port scan: 2 concurrent scans of 500 ports each", verbosity=1)
    ...
    async def _run_parallel_scan(self, target, traceroute_os, timeout_options, nmap_options):
        ...
        # Create individual progress bars for each scan
        first_task_key = f"port_scan_first_half_{target.address}"
        second_task_key = f"port_scan_second_half_{target.address}"
        
        first_task_id = progress_manager.add_task(f"🔍 TCP Ports 1-500 on {target.address}", total=100, task_key=first_task_key)
        second_task_id = progress_manager.add_task(f"🔍 TCP Ports 501-1000 on {target.address}", total=100, task_key=second_task_key)
        
        # Start progress simulation for both scans
        progress_manager.simulate_progress(first_task_id, 300)  # 5 minutes estimate
        progress_manager.simulate_progress(second_task_id, 300)  # 5 minutes estimate
        
        # Run both scans in parallel
        tasks = [
            self._run_port_range_scan(target, traceroute_os, timeout_options, nmap_options, first_half_ports, "first_half", first_task_id),
            self._run_port_range_scan(target, traceroute_os, timeout_options, nmap_options, second_half_ports, "second_half", second_task_id)
        ]
        
        results = await asyncio.gather(*tasks, return_exceptions=True)
        
        # Complete progress bars
        progress_manager.complete_task(first_task_id)
        progress_manager.complete_task(second_task_id)
    ...
    async def _run_port_range_scan(self, target, traceroute_os, timeout_options, nmap_options, port_range, scan_name, task_id=None):
