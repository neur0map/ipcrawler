...
class Enum4Linux(ServiceScan):
    ...
    def configure(self):
        self.add_choice_option(
            "tool",
            default=("enum4linux-ng" if which("enum4linux-ng") else "enum4linux"),
            choices=["enum4linux-ng", "enum4linux"],
            help="The tool to use for doing Windows and Samba enumeration. Default: %(default)s",
        )
    ...
    def check(self):
        ...
        if tool == "enum4linux" and which("enum4linux") is None:
            self.error(
                "The enum4linux program could not be found. Make sure it is installed. (On Kali, run: sudo apt install enum4linux)"
            )
            return False
        elif tool == "enum4linux-ng" and which("enum4linux-ng") is None:
            self.error(
                "The enum4linux-ng program could not be found. Make sure it is installed. (https://github.com/cddmp/enum4linux-ng)"
            )
            return False
        return True
