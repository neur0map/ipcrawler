    ...
    async def run(self, service):
        if self.get_global("domain"):
            await service.execute(
                "dnsrecon -n {address} -d " + self.get_global("domain") + " 2>&1",
                outfile="{protocol}_{port}_dnsrecon_default.txt",
            )
        else:
            service.error(
                "A domain name was not specified in the command line options (--global.domain). If you know the domain name, look in the _manual_commands.txt file for the dnsrecon command."
            )
