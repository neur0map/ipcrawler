    ...
    async def run(self, service):
        if which("redis-cli") is not None:
            _, stdout, _ = await service.execute(
                "redis-cli -p {port} -h {address} INFO", outfile="{protocol}_{port}_redis_info.txt"
            )
            if not (await stdout.readline()).startswith("NOAUTH Authentication required"):
                await service.execute(
                    "redis-cli -p {port} -h {address} CONFIG GET '*'", outfile="{protocol}_{port}_redis_config.txt"
                )
                await service.execute(
                    "redis-cli -p {port} -h {address} CLIENT LIST", outfile="{protocol}_{port}_redis_client-list.txt"
                )
