    ...
    async def run(self, service):
        if service.target.ipversion == "IPv4":
            ...
            await service.execute(
                'smbmap -H {address} -P {port} -x "ipconfig /all" 2>&1', outfile="smbmap-execute-command.txt"
            )
            await service.execute(
                'smbmap -u null -p "" -H {address} -P {port} -x "ipconfig /all" 2>&1', outfile="smbmap-execute-command.txt"
            )
