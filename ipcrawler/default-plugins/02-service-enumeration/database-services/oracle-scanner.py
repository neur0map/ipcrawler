...
class OracleScanner(ServiceScan):
	...
	def check(self):
		...
		if which('oscanner') is None:
			if is_macos:
				self.warn('oscanner not available on macOS. Using nmap Oracle scripts as alternative.')
				return True
			else:
	...
	async def run(self, service):
		...
		if which('oscanner') is not None:
			...
			await service.execute('nmap -p {port} --script oracle-enum-users,oracle-sid-brute,oracle-tns-version {address} 2>&1', outfile='{protocol}_{port}_oracle_nmap.txt')
			# Additional Oracle checks
			await service.execute('nmap -p {port} --script oracle-* {address} 2>&1', outfile='{protocol}_{port}_oracle_full.txt')
