    ...
    async def run(self, service):
        if service.target.ipversion == "IPv4":
            await service.execute(
                "tnscmd10g ping -h {address} -p {port} 2>&1", outfile="{protocol}_{port}_oracle_tnscmd_ping.txt"
            )
            await service.execute(
                "tnscmd10g version -h {address} -p {port} 2>&1", outfile="{protocol}_{port}_oracle_tnscmd_version.txt"
            )
