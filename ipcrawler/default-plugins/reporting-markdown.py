    ...
    async def run(self, targets):
        ...
        for target in targets:
            ...
            files = [
                ...
                for filename in glob.iglob(os.path.join(target.scandir, "**/*"), recursive=True)
                if os.path.isfile(filename) and filename.endswith((".txt", ".html"))
            ]
                    ...
                    if len(target.scans["ports"][scan]["commands"]) > 0:
                        with open(
                            os.path.join(
                                ...
                                target.address,
                                "Port Scans",
                                "PortScan - " + target.scans["ports"][scan]["plugin"].name + ".md",
                            ),
                            "w",
                        ) as output:
                            for command in target.scans["ports"][scan]["commands"]:
                                ...
                                for filename in files:
                                    if (
                                        filename in command[0]
                                        or (command[1] is not None and filename == command[1])
                                        or (command[2] is not None and filename == command[2])
                                    ):
            ...
            if target.scans["services"]:
                ...
                for service in target.scans["services"].keys():
                    os.makedirs(
                        os.path.join(report, target.address, "Services", "Service - " + service.tag().replace("/", "-")),
                        exist_ok=True,
                    )
                        ...
                        if len(target.scans["services"][service][plugin]["commands"]) > 0:
                            with open(
                                os.path.join(
                                    ...
                                    target.address,
                                    "Services",
                                    "Service - " + service.tag().replace("/", "-"),
                                    target.scans["services"][service][plugin]["plugin"].name + ".md",
                                ),
                                "w",
                            ) as output:
