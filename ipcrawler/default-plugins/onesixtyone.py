...
class OneSixtyOne(ServiceScan):
    ...
    def configure(self):
        ...
        self.add_option(
            "community-strings",
            default="/usr/share/seclists/Discovery/SNMP/common-snmp-community-strings-onesixtyone.txt",
    ...
    async def run(self, service):
        if service.target.ipversion == "IPv4":
            await service.execute(
                "onesixtyone -c " + self.get_option("community-strings") + " -dd {address} 2>&1",
                outfile="{protocol}_{port}_snmp_onesixtyone.txt",
            )
