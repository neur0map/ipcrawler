    ...
    async def run(self, service):
        if service.protocol == "tcp":
            process, stdout, _ = await service.execute(
                "curl -sSikf {http_scheme}://{addressv6}:{port}/.well-known/security.txt",
                future_outfile="{protocol}_{port}_{http_scheme}_known-security.txt",
            )
            ...
            if process.returncode == 0 and lines:
                ...
                service.info(
                    "{bblue}["
                    + fformat("{tag}")
                    + "]{rst} There did not appear to be a .well-known/security.txt file in the webroot (/)."
                )
