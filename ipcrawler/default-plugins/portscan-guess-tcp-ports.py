        ...
        if target.ports:
            if target.ports["tcp"]:
                process, stdout, stderr = await target.execute(
                    "nmap {nmap_extra} -A --osscan-guess --version-all -p "
                    + target.ports["tcp"]
                    + ' -oN "{scandir}/_custom_ports_tcp_nmap.txt" -oX "{scandir}/xml/_custom_ports_tcp_nmap.xml" {address}',
                    blocking=False,
                )
            else:
                return []
            ...
            process, stdout, stderr = await target.execute(
                'nmap {nmap_extra} -A --osscan-guess --version-all -p- -oN "{scandir}/_quick_tcp_nmap.txt" -oX "{scandir}/xml/_quick_tcp_nmap.xml" {address}',
                blocking=False,
            )
