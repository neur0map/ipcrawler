...
class SubdomainEnumeration(ServiceScan):
    ...
    def configure(self):
        ...
        self.add_list_option(
            ...
            help="The wordlist(s) to use when enumerating subdomains. Separate multiple wordlists with spaces. Default: %(default)s",
        )
        self.add_option(
    ...
    async def run(self, service):
        ...
        if len(domains) > 0:
            ...
            service.info(
                "The target was not a domain, nor was a domain provided as an option. Skipping subdomain enumeration."
            )
