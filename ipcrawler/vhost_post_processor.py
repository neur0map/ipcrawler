...
class VHostPostProcessor:
                            ...
                            for line in content.split("\n"):
                                if line.startswith("Extracted Hostname:"):
                                    ...
                                    if hostname and hostname != ip:
                                        self.discovered_vhosts.append({"hostname": hostname, "ip": ip, "file": file_path})
    ...
    def add_hosts_entries(self, entries_to_add):
        ...
        try:
            ...
            # Prepare the command
            entries_text = "\n".join(
                ["\n# Added by ipcrawler VHost discovery", f"# Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"]
                + temp_entries
                + [""]
            )
    ...
    def run_interactive_session(self):
        ...
        new_vhosts = [v for v in self.discovered_vhosts if v["hostname"] not in self.existing_hosts]
